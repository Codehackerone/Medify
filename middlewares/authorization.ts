...
export const authorize = () => {
    return async (req:any, res:any, next:any) => {
      if (
        ...
        return messageError(res, BAD_REQUEST, "No auth token found", "AuthenticationError");
      } else {
        try {
          let decoded:any = "";
          let authorizationHeaderArray= req.headers["authorization"].split(" ");
          if(authorizationHeaderArray[0] !== "Bearer"){
            return messageError(res, BAD_REQUEST, "We dont accept any token other than Bearer", "AuthenticationError");
          }
          decoded = jwt.verify(
